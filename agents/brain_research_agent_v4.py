...
def load_queued_tasks(
    ...
    """Load research tasks with strategy-level paper-gap work first."""
    paper_rows = conn.execute(
        ...
        WHERE TRIM(LOWER(COALESCE(status, ''))) = 'queued'
          AND (
              UPPER(COALESCE(metadata_json, '')) LIKE '%PAPER_EVIDENCE_GAP_RESEARCH%'
              OR LOWER(COALESCE(question, '')) LIKE 'collect independent strategy-level evidence%'
          )
    ...
    # Fallback: load all other eligible queued research tasks.
    rows = conn.execute(
        """
        SELECT *
        FROM brain_research_queue
        ...
        LIMIT ?
        """,
        (max(BATCH_SIZE * 100, 1000),),
    ).fetchall()

    eligible: list[sqlite3.Row] = list(selected)
    selected_ids = {safe_int(row["id"]) for row in selected}
...
def enrich_paper_gap_metadata(
    conn: sqlite3.Connection,
    metadata: dict[str, Any],
    question: str,
) -> dict[str, Any]:
    """Recover original paper-evidence metadata for a strategy-level gap task.

    Older grouped queue rows can carry symbols but miss strategy metrics.
    The authoritative prior Research Agent episode is retained in
    brain_episodes, so recover its metadata before synthesizing.
    """
    ...
    add_candidate(merged.get("paper_evidence"))

    episode_ids = []
    for pattern in (
        r"episode[=_\s]+(\d+)",
        r"knowledge_item_id[=_\s]+(\d+)",
        r"knowledge=([0-9]+)",
    ):
        match = re.search(pattern, norm(question), flags=re.IGNORECASE)
        if match:
            episode_ids.append(safe_int(match.group(1)))

    explicit_episode_id = safe_int(
        merged.get("source_episode_id"),
        0,
    )
    if explicit_episode_id:
        episode_ids.insert(0, explicit_episode_id)

    # Prefer a directly referenced research-result episode.
    for episode_id in dict.fromkeys(
        x for x in episode_ids if x > 0
    ):
        row = conn.execute(
            """
            ...
            FROM brain_episodes
            WHERE id=?
              AND episode_type='research_agent_result'
            LIMIT 1
            """,
            (episode_id,),
        ).fetchone()
        ...
        episode_meta = parse_metadata(
            row["metadata_json"]
        )
        add_candidate(episode_meta)
        add_candidate(
            episode_meta.get("paper_evidence")
        )

    # If the queue row references an earlier research queue, recover its
    # metadata directly as a second source.
    source_queue_id = safe_int(
        merged.get("source_queue_id")
        or merged.get("paper_evidence_queue_id"),
        0,
    )
    ...
    if source_queue_id:
        ...
        if row:
            source_meta = parse_metadata(
                row["metadata_json"]
            )
            add_candidate(source_meta)
            add_candidate(
                source_meta.get("paper_evidence")
            )

    # Fill only missing values: the current grouped task remains authoritative
    # where it already has valid evidence.
    priority_keys = (
        "strategy_id",
        "strategy_name",
        "classification",
    ...
    # Preserve an explicit provenance pointer when we recover it.
    if episode_ids:
        merged.setdefault(
            "source_episode_id",
            episode_ids[0],
...
def reconstruct_context(
    ...
    task_type_from_metadata = get_task_type(
        queue_row
    )

    if task_type_from_metadata == "PAPER_EVIDENCE_GAP_RESEARCH":
        metadata = enrich_paper_gap_metadata(
            conn,
            metadata,
            norm(queue_row["question"]),
        )

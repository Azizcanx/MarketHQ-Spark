# -*- coding: utf-8 -*-
"""
MarketHQ Fresh Independent Evidence -> Knowledge Bridge V1
...
EVIDENCE_DIR = PROJECT_ROOT / "fresh_independent_research_results"
...
def find_latest_evidence() -> Path:
    files = sorted(
        EVIDENCE_DIR.glob("fresh_independent_*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    if not files:
        raise FileNotFoundError(
            f"Fresh independent evidence JSON bulunamadı: {EVIDENCE_DIR}"
        )
    return files[0]
...
def normalize_results(
    ...
    """
    Fresh Research Engine V1.1 stores `results` as a list of
    symbol-result records. Older independent evidence used a dict.

    Normalize both shapes into:
        [(symbol, payload), ...]
    ...
    if isinstance(results, dict):
        for symbol, payload in results.items():
            if isinstance(payload, dict):
                normalized.append((norm(symbol), payload))
        return normalized
    ...
    if isinstance(results, list):
        for index, record in enumerate(results, start=1):
            ...
            if not symbol:
                # Preserve the record even when the symbol field is absent;
                # this makes schema problems visible instead of crashing.
                symbol = f"UNKNOWN_{index:02d}"
...
def build_content(evidence: dict[str, Any]) -> str:
    ...
    results = normalize_results(evidence.get("results", []))
    ...
    for symbol, payload in results:
        ...
        # V1.1 fresh engine stores each record directly; the bridge also
        # supports the older nested independent-evidence shape.
        full = payload.get("full_backtest", {})
        if not isinstance(full, dict):
            full = {}

        with_costs = full.get("with_costs", {})
        if not isinstance(with_costs, dict):
            with_costs = {}

        cost_free = full.get("cost_free", {})
        if not isinstance(cost_free, dict):
            cost_free = {}
        ...
        # Fresh engine may expose metrics directly on the record.
        if not with_costs and isinstance(payload.get("metrics"), dict):
            with_costs = {"metrics": payload.get("metrics", {})}

        if with_costs.get("metrics") is None:
            with_costs["metrics"] = {}

        if cost_free.get("metrics") is None:
            cost_free["metrics"] = {}
        ...
        holdout_payload = {
            "status": rolling.get("status", payload.get("holdout_status")),
            "total_folds": rolling.get(
                "total_folds",
                payload.get("total_holdout_folds"),
            ),
            "positive_folds": rolling.get(
                "positive_folds",
                payload.get("positive_holdout_folds"),
            ),
            "negative_folds": rolling.get(
                "negative_folds",
                payload.get("negative_holdout_folds"),
            ),
        }

        # Also preserve the common V1.1 top-level holdout ratio fields when
        # present, without inventing missing values.
        for key in (
            "positive_ratio",
            "holdout_return",
            "return_percent",
            ...
            if key in payload:
                holdout_payload[key] = payload.get(key)

        lines.append(
            "holdout="
            + compact_json(holdout_payload)
        )
...
def insert_or_upgrade_item(
    ...
    metadata = {
        "engine": "MARKETHQ_FRESH_INDEPENDENT_RESEARCH_ENGINE",
        "bridge": "MARKETHQ_FRESH_INDEPENDENT_EVIDENCE_KNOWLEDGE_BRIDGE",
        "bridge_version": "FRESH_V1",
    ...
    if existing:
        ...
        if "summary" in cols:
            ...
            values.append(
                "Fresh independent historical evidence; not verified."
            )
    ...
    add(
        "summary",
        "Fresh independent strategy evidence; not verified.",
    )
...
def main() -> None:
    ...
    try:
        ...
        print("=" * 76)
        print("MARKETHQ FRESH INDEPENDENT EVIDENCE → KNOWLEDGE BRIDGE V1")
        print("=" * 76)

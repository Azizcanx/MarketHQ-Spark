# -*- coding: utf-8 -*-
"""
MarketHQ Final Research Decision Engine - V3
==================================================
...
def _reconcile_full_run_counts_from_content(
    knowledge: dict[str, Any],
) -> tuple[int, int, int, str]:
    """
    Reconcile Fresh evidence counts from the persisted knowledge content.

    Fresh Research V1.1 can store per-symbol results as a list and the
    bridge preserves those results in `content`. Some older summary payloads
    do not expose the canonical `positive_full_runs` field, so a strategy
    decision must not silently turn a known 11/16 result into 0/16.

    Returns: (symbols, positive, negative, source_method)
    """
    ...
    for raw_line in content.splitlines():
        ...
        if pending_symbol and line.startswith("with_costs="):
            pending_symbol = False
            payload_text = line[len("with_costs="):].strip()
            try:
                metrics = json.loads(payload_text)
            except (TypeError, ValueError, json.JSONDecodeError):
                continue
            ...
            if return_percent > 0 or (return_percent == 0 and net_pnl > 0):
                positive += 1
            elif return_percent < 0 or net_pnl < 0:
                negative += 1
    ...
    if symbols > 0 and positive + negative == symbols:
        return symbols, positive, negative, "knowledge_content_symbol_metrics"
...
def build_independent_metrics(
    ...
    reconciliation_method = "summary"

    # The Fresh V1.1 result may carry the real 11/16 outcome only inside the
    # per-symbol metrics preserved by the knowledge bridge. Reconcile from
    # those records when the summary lacks a usable positive/negative split.
    if symbols_tested > 0 and (positive_full_runs + negative_full_runs) == 0:
        rec_symbols, rec_positive, rec_negative, rec_method = (
            _reconcile_full_run_counts_from_content(knowledge)
        )
        if rec_symbols > 0:
            symbols_tested = rec_symbols
            positive_full_runs = rec_positive
            negative_full_runs = rec_negative
            reconciliation_method = rec_method
    ...
    if positive_full_runs + negative_full_runs > symbols_tested:
        raise RuntimeError(
            "Independent evidence: full-run counts symbols_tested değerini aşıyor."
        )
    ...
    return {
        ...
        "negative_full_runs": negative_full_runs,
        ...
        "full_run_count_source": reconciliation_method,
    }
...
def evaluate_decision(
    ...
    holdout_available = bool(
        independent.get("holdout_available", False)
    )
    raw_holdout = independent.get("holdout_ratio")
    holdout = (
        clamp01(safe_float(raw_holdout))
        if holdout_available and raw_holdout is not None
        else None
    )
    full_run = clamp01(
    ...
    if holdout is not None:
        weighted_parts.append((WEIGHT_HOLDOUT, holdout))

    weight_total = sum(weight for weight, _ in weighted_parts)
    composite = (
        sum(weight * value for weight, value in weighted_parts)
        / weight_total
        if weight_total > 0
        else 0.0
    )
    ...
    if holdout is not None and holdout < 0.55:
        reasons.append(
        ...
        reasons.append(
            "Fresh independent evidence does not contain rolling holdout folds; "
            "holdout score is excluded rather than treated as zero."
        )
    ...
    return {
        ...
        "independent_holdout_ratio": (
            round(holdout, 4) if holdout is not None else None
        ),
        "independent_holdout_available": holdout_available,
        "independent_holdout_total": safe_int(
            independent.get("total_holdout_folds")
        ),
        "independent_holdout_positive": safe_int(
            independent.get("positive_holdout_folds")
        ),
...
def print_report(
    ...
    print("=" * 78)
    print("MARKETHQ FINAL RESEARCH DECISION ENGINE V3")
    print("=" * 78)
    print()
    ...
    print(
        "Full-run count source    : "
        f"{independent.get('full_run_count_source', 'unknown')}"
    )
    if independent.get("holdout_available"):
        holdout_text = (
            f"{independent['positive_holdout_folds']}/"
            f"{independent['total_holdout_folds']}"
        )
    else:
        holdout_text = "NOT_AVAILABLE"

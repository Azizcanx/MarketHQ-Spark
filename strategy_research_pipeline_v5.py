...
"""
MarketHQ - Strategy Research Pipeline V5
=========================================
...
def run_walk_forward(
    ...
    while (
        ...
        print(
            f"Train      : {len(train)} bars"
        )

        print(
            f"Validation : {len(validation)} bars"
        )

        train_screen = fast_screen(
            train,
            symbol,
            candidates,
        )

        top_candidates = train_screen[
            :TOP_WFO_CANDIDATES
        ]
    ...
    return {
        ...
        "validation_return_std": (
            return_std
        ),
        "total_validation_pnl": sum(
            pnls
        ),
        "folds": folds,
    }
...
def research_strategy(
    ...
    print_header(
        f"📊 V5 STRATEGY SUMMARY: "
        f"{strategy_name}"
    )

    print(
        f"Backtest symbols       : "
        f"{consistency['symbols']}"
    )

    print(
        f"Positive symbols       : "
        f"{consistency['positive']}"
    )

    print(
        f"Negative symbols       : "
        f"{consistency['negative']}"
    )

    print(
        f"Cross-symbol ratio     : "
        f"{consistency['positive_ratio']:.2%}"
    )

    print(
        f"Average return         : "
        f"{consistency['return_mean']:.2f}%"
    )

    print(
        f"Return std             : "
        f"{consistency['return_std']:.2f}%"
    )

    print()

    total_positive_wfo = sum(
        safe_int(
            item.get(
                "positive_folds"
            )
    ...
    print(
        f"WFO positive folds    : "
        f"{total_positive_wfo}"
    )

    print(
        f"WFO negative folds    : "
        f"{total_negative_wfo}"
    )

    print(
        f"WFO inconclusive      : "
        f"{total_inconclusive_wfo}"
    )
    ...
    print(
        f"Cost robustness       : "
        f"{ranking['cost_survival']:.2%}"
    )

    print(
        f"Parameter stability   : "
        f"{ranking['parameter_stability']:.2%}"
    )

    print(
        f"Regime stability      : "
        f"{ranking['regime_stability']:.2%}"
    )

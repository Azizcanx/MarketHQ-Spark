...
export async function runResearchDateResolverAgent(
  ...
  // AgentContext.input is intentionally generic in the current MarketHQ types.
  // Normalize it before reading datePolicy so TypeScript does not infer
  // datePolicy as an empty object and raise TS2339 on its properties.
  ...
  const lookbackBars = Math.max(
    ...
    Math.floor(toNumber(datePolicy.lookbackBars, DEFAULT_LOOKBACK_BARS)),
  );
  const trainBars = Math.max(
    DEFAULT_TRAIN_BARS,
    Math.floor(toNumber(datePolicy.trainBars, DEFAULT_TRAIN_BARS)),
  );
  ...
  const independentBars = Math.max(
    ...
    Math.floor(toNumber(datePolicy.independentBars, DEFAULT_INDEPENDENT_BARS)),
  );
